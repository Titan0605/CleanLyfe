...
def carbonfp():
    if 'username' in session:
        user = session
        return render_template('carbonfp.html', user=user)
    else:
...
def carbonfp_transport():
    if 'username' in session:
        user = session
        return render_template('carbonfp_transport.html', user=user)
    else:
       return redirect(url_for('indexes_views.index_views'))
...
def carbonfp_devices():
    if 'username' in session: 
        ...
        return render_template('carbonfp_energy.html', devices=devices, user=user)
    else:
        return redirect(url_for('indexes_views.index_views'))
...
def carbonfp_products():
    if 'username' in session:
        user = session
        return render_template('carbonfp_products.html', user=user)
    else:
       return redirect(url_for('indexes_views.index_views'))
...
def carbonfp_water():
    if 'username' in session:
        user = session
        return render_template('carbonfp_water.html', user=user)
    else:
       return redirect(url_for('indexes_views.index_views'))